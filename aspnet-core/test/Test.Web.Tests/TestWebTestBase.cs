...
{
    public abstract class TestWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static TestWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(TestWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<TestDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<TestDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<TestDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<TestDbContext, Task<T>> func)
        {
            T result;
